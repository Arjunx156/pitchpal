...
- **Performance** — only the relevant slice of venue data is grounded into the prompt; answers stream token-by-token; measured payload: ~150 kB gzipped JS total (app 67 kB + React 46 kB + motion 38 kB vendor chunks that cache independently) and 11 kB gzipped CSS; fonts are self-hosted, subsetted, and gated by `unicode-range` so a fan only downloads the scripts they render (Arabic loads only in Arabic); the offline precache skips subsets no UI language uses; and the map, ops, forecast, standings and itinerary are all computed from pure functions with no extra network round-trips (`npm run analyze` renders the bundle treemap).
